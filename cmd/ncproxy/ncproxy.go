...
// TODO katiewasnothere: this needs to be defined in compute agent also, consider moving
func init() {
	typeurl.Register(&networking.NCProxyEndpoint{}, "ncproxy/networking/CustomEndpoint")
	typeurl.Register(&networking.NCProxyNetwork{}, "ncproxy/networking/CustomNetwork")
...
type grpcService struct {
	...
	// TODO katiewasnothere: code for repopulating this
	ccn *customComputeNetworkingCache
}

func newGRPCService(agentCache *computeAgentCache) *grpcService {
	return &grpcService{
		containerIDToComputeAgent: agentCache,
		ccn:                       newCustomComputeNetworkingCache(),
	}
}
...
func (s *grpcService) AddNIC(ctx context.Context, req *ncproxygrpc.AddNICRequest) (_ *ncproxygrpc.AddNICResponse, err error) {
	...
	if endpt, ok := s.ccn.endpoints.get(req.EndpointName); ok {
...
func (s *grpcService) ModifyNIC(ctx context.Context, req *ncproxygrpc.ModifyNICRequest) (_ *ncproxygrpc.ModifyNICResponse, err error) {
	...
	if _, ok := s.ccn.endpoints.get(req.EndpointName); ok {
...
func (s *grpcService) CreateNetwork(ctx context.Context, req *ncproxygrpc.CreateNetworkRequest) (_ *ncproxygrpc.CreateNetworkResponse, err error) {
	...
	case *ncproxygrpc.Network_NcproxyNetwork:
		...
		// TODO katiewasnothere: move to endpoint creation, check if pci option set
		/*agent, err := s.containerIDToComputeAgent.get(req.ContainerID)
		if err == nil {
			caReq := &computeagent.AssignPCIInternalRequest{
				ContainerID:          req.ContainerID,
				DeviceID:             settings.DeviceID,
				VirtualFunctionIndex: settings.VirtualFunctionIndex,
			}
			if _, err := agent.AssignPCI(ctx, caReq); err != nil {
				return nil, err
			}
		}*/
		network := &networking.NCProxyNetwork{
			NetworkName: settings.Name,
			Settings:    settings,
		}
		s.ccn.networks.put(settings.Name, network)
		// TODO katiewasnothere: add it to the store as well
		return &ncproxygrpc.CreateNetworkResponse{
			ID: settings.Name,
		}, nil
	}
...
func (s *grpcService) CreateEndpoint(ctx context.Context, req *ncproxygrpc.CreateEndpointRequest) (_ *ncproxygrpc.CreateEndpointResponse, err error) {
	...
	if req.ContainerID == "" || req.EndpointSettings == nil {
		return nil, status.Errorf(codes.InvalidArgument, "received empty field in request: %+v", req)
	}
	...
	case *ncproxygrpc.EndpointSettings_NcproxyEndpoint:
		...
		network, ok := s.ccn.networks.get(reqEndpoint.NetworkName)
		if !ok || network == nil {
			return nil, errors.Errorf("network %v does not exist", reqEndpoint.NetworkName)
		}
		ep := &networking.NCProxyEndpoint{
			EndpointName: reqEndpoint.Name,
			Settings:     reqEndpoint,
		}

		s.ccn.endpoints.put(reqEndpoint.Name, ep)
		return &ncproxygrpc.CreateEndpointResponse{
			ID: reqEndpoint.Name,
		}, nil
	}
...
func (s *grpcService) AddEndpoint(ctx context.Context, req *ncproxygrpc.AddEndpointRequest) (_ *ncproxygrpc.AddEndpointResponse, err error) {
	...
	if endpt, ok := s.ccn.endpoints.get(req.Name); ok {
		endpt.Add(req.NamespaceID)
	} else {
		// see if this is an hcn endpoint
...
func (s *grpcService) DeleteEndpoint(ctx context.Context, req *ncproxygrpc.DeleteEndpointRequest) (_ *ncproxygrpc.DeleteEndpointResponse, err error) {
	...
	if _, ok := s.ccn.endpoints.get(req.Name); ok {
		s.ccn.endpoints.delete(req.Name)
	} else {
		ep, err := hcn.GetEndpointByName(req.Name)
		if err != nil {
			if _, ok := err.(hcn.EndpointNotFoundError); ok {
...
func (s *grpcService) DeleteNetwork(ctx context.Context, req *ncproxygrpc.DeleteNetworkRequest) (_ *ncproxygrpc.DeleteNetworkResponse, err error) {
	...
	if _, ok := s.ccn.networks.get(req.Name); ok {
		s.ccn.networks.delete(req.Name)
	} else {
...
// TODO katiewasnothere: ask sushant's team what they want returned here
func (s *grpcService) GetEndpoint(ctx context.Context, req *ncproxygrpc.GetEndpointRequest) (_ *ncproxygrpc.GetEndpointResponse, err error) {
	...
	if endpt, ok := s.ccn.endpoints.get(req.Name); ok {
		// TODO katiewasnothere: what to do for Name and ID for custom
		return &ncproxygrpc.GetEndpointResponse{
			Endpoint: &ncproxygrpc.GetEndpointResponse_NcproxyEndpoint{
				NcproxyEndpoint: &ncproxygrpc.NCProxyEndpointResponse{
					Namespace: endpt.NamespaceID,
					Settings:  endpt.Settings,
				},
			},
		}, nil
	} else {
		...
		return &ncproxygrpc.GetEndpointResponse{
			Endpoint: &ncproxygrpc.GetEndpointResponse_HcnEndpoint{
				HcnEndpoint: &ncproxygrpc.HcnEndpointResponse{
					Namespace: ep.HostComputeNamespace,
					Settings: &ncproxygrpc.HcnEndpointSettings{
						Name:       req.Name,
	...
	return nil, status.Errorf(codes.FailedPrecondition, "no endpoint found for %v", req.Name)
}
...
func (s *grpcService) GetEndpoints(ctx context.Context, req *ncproxygrpc.GetEndpointsRequest) (_ *ncproxygrpc.GetEndpointsResponse, err error) {
	...
	for _, endpoint := range s.ccn.endpoints.getAll() {
		endpointReq := &ncproxygrpc.GetEndpointRequest{
...
func (s *grpcService) GetNetwork(ctx context.Context, req *ncproxygrpc.GetNetworkRequest) (_ *ncproxygrpc.GetNetworkResponse, err error) {
	...
	if network, ok := s.ccn.networks.get(req.Name); ok {
		// TODO katiewasnothere: remove network from cache and store
		s.ccn.networks.delete(req.Name)
		return &ncproxygrpc.GetNetworkResponse{
			...
			Network: &ncproxygrpc.Network{
				Settings: &ncproxygrpc.Network_NcproxyNetwork{
					NcproxyNetwork: network.Settings,
				},
			},
		}, nil
	}
		...
		Network: &ncproxygrpc.Network{
			Settings: &ncproxygrpc.Network_HcnNetwork{
				HcnNetwork: &ncproxygrpc.HostComputeNetworkSettings{
					Name: req.Name,
					// TODO katiewasnothere: what do we do here
				},
			},
		},
	}, nil
...
func (s *grpcService) GetNetworks(ctx context.Context, req *ncproxygrpc.GetNetworksRequest) (_ *ncproxygrpc.GetNetworksResponse, err error) {
	...
	for _, network := range s.ccn.networks.getAll() {
		networkReq := &ncproxygrpc.GetNetworkRequest{
			Name: network.NetworkName,
		}
		n, err := s.GetNetwork(ctx, networkReq)
