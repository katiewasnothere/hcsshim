...
func install() error {
	...
	if _, err := os.Stat(moduleDir); err != nil {
		if err := os.MkdirAll(moduleDir, 0700); err != nil {
			return errors.Wrap(err, "failed to make the directory")
		}
	}
	...
	for _, driver := range args {
		...
		fmt.Fprintln(os.Stderr, targetModuleDirPath)
		...
		cmd := exec.Command("ls", driverModulePath)
		...
		if walkErr := filepath.WalkDir(driverModulePath, func(path string, d fs.DirEntry, err error) error {
			...
			fmt.Fprintln(os.Stderr, d.IsDir())
