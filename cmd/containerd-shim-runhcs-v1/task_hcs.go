...
import (
	...
	"github.com/Microsoft/hcsshim/internal/resourcepaths"
...
func (ht *hcsTask) Update(ctx context.Context, req *task.UpdateTaskRequest) error {
	...
	if ht.ownsHost && ht.host != nil {
		if err := ht.host.UpdateConstraints(ctx, resources, req.Annotations); err != nil {
			return err
		}
	} else {
...
func (ht *hcsTask) updateWCOWContainerCPU(ctx context.Context, cpu *specs.WindowsCPUResources) error {
	// Add utility tool to container host if not present
	hostPath, guestPath := getToolPaths()
	shareReq := &shimdiag.ShareRequest{
		HostPath: hostPath,
		UvmPath:  guestPath,
		ReadOnly: true,
	}
	err := ht.Share(ctx, shareReq)
	...
	args := createJobObjectsUtilArgs(ctx, guestPath, cpu)
	req := &shimdiag.ExecProcessRequest{
		Args: args,
	}
	if exitCode, err := ht.ExecInHost(ctx, req); err != nil {
