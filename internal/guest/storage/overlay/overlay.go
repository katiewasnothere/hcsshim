...
func MountOverlay(ctx context.Context, layerPaths []string, upperdirPath, workdirPath, rootfsPath string, readonly bool) (err error) {
	_, span := trace.StartSpan(ctx, "overlay::MountOverlay")
	defer span.End()
	defer func() { oc.SetSpanStatus(span, err) }()
