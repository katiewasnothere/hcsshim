...
func CreateContainer(ctx context.Context, createOptions *CreateOptions) (_ cow.Container, _ *resources.Resources, err error) {
	...
	if coi.HostingSystem != nil {
		...
		if coi.Spec.Linux != nil {
			r.ContainerRootInUVM = fmt.Sprintf(lcowRootInUVM, createOptions.ID)
		} else {
			r.ContainerRootInUVM = fmt.Sprintf(wcowRootInUVM, strconv.FormatUint(n, 16))
		}
	}
...
// hasAssignedWindowsDevices returns true if the creation options contain windows devices
func (coi *createOptionsInternal) hasAssignedWindowsDevices() bool {
